...
class RemoteCacheSettings {
	...
	public function admin_menu() {
		add_menu_page( __( 'Remote Cache Purger', 'varnish-http-purge' ), __( 'Proxy Cache', 'varnish-http-purge' ), 'manage_options', 'varnish-page', array( &$this, 'settings_page' ), RemoteCachePurger::get_icon_svg( true, '#82878c' ), 75 );
		add_submenu_page( 'varnish-page', __( 'Remote Cache Purger', 'varnish-http-purge' ), __( 'Settings', 'varnish-http-purge' ), 'manage_options', 'varnish-page', array( &$this, 'settings_page' ) );
	...
	public function settings_devmode_callback() {
		...
		$devmode = get_site_option( 'vhp_varnish_devmode', RemoteCachePurger::$devmode );
	...
	public function settings_page() {
		...
		<div class="wrap">
			<?php settings_errors(); ?>
			<h1><?php esc_html_e( 'Remote Cache Purger Settings', 'varnish-http-purge' ); ?></h1>

			<p><?php esc_html_e( 'Remote Cache Purger can empty the cache for different server based caching systems, including Varnish and nginx. For most users, there should be no configuration necessary as the plugin is intended to work silently, behind the scenes.', 'varnish-http-purge' ); ?></p>
