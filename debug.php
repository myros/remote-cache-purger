...
class RemoteCacheDebug {
	...
	public static function devmode_toggle( $state = 'deactivate' ) {
		...
		update_site_option( 'remote_cache_devmode', $newmode );
	...
	public static function is_url_valid( $input ) {
		...
		$default = esc_url( RemoteCachePurger::the_home_url() );
	...
	public static function preflight( $response ) {
		...
		// Defaults.
		$preflight = true;
		$message   = __( 'Success', 'rmeote-cache-purge' );

		if ( is_wp_error( $response ) ) {
			$preflight = false;
			$message   = __( 'This request cannot be performed: ', 'rmeote-cache-purge' );
			$message  .= $response->get_error_message();
		} elseif ( '404' === wp_remote_retrieve_response_code( $response ) ) {
			$preflight = false;
			$message   = __( 'This URL does not resolve properly. Either it\'s was not found or it redirects incorrectly.', 'rmeote-cache-purge' );
		}
	...
	public static function varnish_results( $headers = false ) {
		...
		if ( ! $headers ) {
			...
			if ( false !== $cache_service ) {
				...
				$return['message'] = sprintf( __( 'Your %1 caching service appears to be running properly.', 'rmeote-cache-purge' ), $cache_service );
				$return['icon']    = 'good';
			}
		}
		...
		if ( ! $cache_service ) {
			$return['icon']    = 'bad';
			$return['message'] = __( 'No known cache service has been detected on your site.', 'rmeote-cache-purge' );
		} elseif ( ! $kronk ) {
			$return['icon']    = 'bad';
			$return['message'] = __( 'Your site is not responding. If this happens again, please contact your webhost.', 'rmeote-cache-purge' );
		} elseif ( ! $cacheheaders_set ) {
			$return['icon']    = 'notice';
			$return['message'] = __( 'We were unable find a caching service active for this domain. This may occur if you use a proxy service (such as CloudFlare or Sucuri) or if you\'re in the middle of a DNS move.', 'rmeote-cache-purge' );
		} elseif ( $is_cachable && $still_cachable ) {
			$return['icon'] = 'awesome';
		} else {
			// translators: %1 is the type of caching service detected (i.e. nginx or varnish).
			$return['message'] = sprintf( __( '%s caching service is running but is unable to cache your site.', 'rmeote-cache-purge' ), $cache_service );
			$return['icon']    = 'warning';
		}
	...
	public static function remote_ip_results( $remote_ip, $varniship, $headers ) {
		...
		if ( $x_nginx && 'localhost' === $varniship ) {
			$return = array(
				'icon'    => 'awesome',
				'message' => __( 'Your Nginx Proxy is set up correctly.', 'rmeote-cache-purge' ),
			);
		} elseif ( false === $remote_ip && ! empty( $varniship ) ) {
			$return = array(
				// translators: %s is an IP address.
				'message' => sprintf( __( 'Your Varnish IP address is set to %s but a proxy (like Cloudflare or Sucuri) has not been detected. This is mostly harmless, but if you have issues with your cache not emptying when you make a post, you may need to remove your Varnish IP. Please check with your webhost or server admin before doing so.', 'rmeote-cache-purge' ), $varniship ),
				'icon'    => 'warning',
			);
		} elseif ( false !== $remote_ip && $remote_ip !== $varniship ) {
			$return = array(
				'icon'    => 'warning',
				'message' => __( 'You\'re using a custom Varnish IP that doesn\'t appear to match your server IP address. If you\'re using multiple caching servers or IPv6, this is fine. Please make sure you\'ve properly configured it according to your webhost\'s specifications.', 'rmeote-cache-purge' ),
			);
		} else {
			$return = array(
				'icon'    => 'awesome',
				'message' => __( 'Your server IP setup looks good.', 'rmeote-cache-purge' ),
			);
		}
	...
	public static function server_results( $headers ) {
		...
		if ( isset( $headers['Server'] ) ) {
			...
			if ( strpos( $headers['Server'], 'Apache' ) !== false && strpos( $headers['Server'], 'cloudflare' ) === false ) {
				$return['Apache'] = array(
					'icon'    => 'awesome',
					'message' => __( 'Your server is running Apache.', 'rmeote-cache-purge' ),
				);
			}
			...
			if ( strpos( $headers['Server'], 'nginx' ) !== false && strpos( $headers['Server'], 'cloudflare' ) === false ) {
				$return['Nginx'] = array(
					'icon'    => 'awesome',
					'message' => __( 'Your server is running Nginx.', 'rmeote-cache-purge' ),
				);
			}
			...
			if ( strpos( $headers['Server'], 'cloudflare' ) !== false ) {
				$return['CloudFlare'] = array(
					'icon'    => 'warning',
					'message' => __( 'CloudFlare has been detected. Make sure you configure WordPress properly by adding your Varnish IP and to flush the CloudFlare cache if you see inconsistencies.', 'rmeote-cache-purge' ),
				);
			}
			...
			if ( isset( $headers['X-Powered-By'] ) && strpos( $headers['X-Powered-By'], 'HHVM' ) !== false ) {
				$return['HHVM'] = array(
					'icon'    => 'notice',
					'message' => __( 'You are running HHVM instead of PHP. While that is compatible with Varnish, you should consider PHP 7. WordPress will cease support for HHVM in 2018.', 'rmeote-cache-purge' ),
				);
			}
			...
			if ( strpos( $headers['Server'], 'Pagely' ) !== false ) {
				$return['Pagely'] = array(
					'icon'    => 'good',
					'message' => __( 'This site is hosted on Pagely. The results of this scan may not be accurate.', 'rmeote-cache-purge' ),
				);
			}
		}
		...
		if ( isset( $headers['X-hacker'] ) ) {
			$return['WordPress.com'] = array(
				'icon'    => 'bad',
				'message' => __( 'This site is hosted on WordPress.com. The results of this scan may not be accurate.', 'rmeote-cache-purge' ),
			);
		}
		...
		if ( isset( $headers['X-Backend'] ) && strpos( $headers['X-Backend'], 'wpaas_web_' ) !== false ) {
			$return['GoDaddy'] = array(
				'icon'    => 'good',
				'message' => __( 'This site is hosted on GoDaddy. The results of this scan may not be accurate.', 'rmeote-cache-purge' ),
			);
		}
	...
	public static function gzip_results( $headers ) {
		...
		if ( strpos( $headers['Content-Encoding'], 'gzip' ) !== false || ( isset( $headers['Vary'] ) && strpos( $headers['Vary'], 'gzip' ) !== false ) ) {
			$return = array(
				'icon'    => 'good',
				'message' => __( 'Your site is compressing content and making the internet faster.', 'rmeote-cache-purge' ),
			);
		}
		...
		if ( strpos( $headers['Content-Encoding'], 'Fastly' ) !== false ) {
			$return = array(
				'icon'    => 'good',
				'message' => __( 'Fastly is speeding up your site. Remember to empty all caches in all locations when necessary.', 'rmeote-cache-purge' ),
			);
		}
	...
	public static function cookie_results( $headers ) {
		...
		if ( ! isset( $headers['Set-Cookie'] ) ) {
			$return['No Cookies'] = array(
				'icon'    => 'awesome',
				'message' => __( 'No active cookies have been detected on your site. You may safely ignore any warnings about cookies set by plugins or themes, as your server has properly accounted for them.', 'rmeote-cache-purge' ),
			);
		} else {
			// We have at least one cookie, so let's set this now.
			$return['Cookies Found'] = array(
				'icon'    => 'warning',
				'message' => __( 'Cookies have been detected. Unless your caching service is configured properly for the specific cookies, it may not cache properly. Please contact your webhost or administrator with information about the cookies found.', 'rmeote-cache-purge' ),
			);

			// Let's check our known bad cookies.
			$request = wp_remote_get( 'https://rmeote-cache-purge.objects-us-east-1.dream.io/cookies.json' );
			...
			if ( empty( $cookies ) ) {
				if ( WP_DEBUG ) {
					$return['cookie-error'] = array(
						'icon'    => 'warning',
						'message' => __( 'Error: Cookie data cannot be loaded.', 'rmeote-cache-purge' ),
					);
				}
	...
	public static function cache_results( $headers ) {
		...
		if ( isset( $headers['Cache-Control'] ) ) {
			...
			if ( $no_cache ) {
				$return['No Cache Header'] = array(
					'icon'    => 'bad',
					'message' => __( 'The header Cache-Control is returning "no-cache", which means visitors will never get cached pages.', 'rmeote-cache-purge' ),
				);
			}
			...
			if ( $max_age ) {
				$return['max_age'] = array(
					'icon'    => 'bad',
					'message' => __( 'The header Cache-Control is returning "max-age=0", which means a page can be no older than 0 seconds before it needs to regenerate the cache.', 'rmeote-cache-purge' ),
				);
			}
		}
		...
		if ( ! isset( $headers['Age'] ) ) {
			$return['Age Headers'] = array(
				'icon'    => 'bad',
				'message' => __( 'Your domain does not report an "Age" header, making it impossible to determine if the page is actually serving from cache.', 'rmeote-cache-purge' ),
			);
			...
			$return['Age Headers'] = array(
				...
				'message' => sprintf( __( 'The "Age" header is returning %s, which means it is not properly caching. Either this URL is intentionally excluded from caching, or a theme or plugin is instructing WordPress not to cache.', 'rmeote-cache-purge' ), $age_header ),
				'icon'    => 'warning',
			);
		} elseif ( (bool) strtotime( $headers['Age'] ) && time() <= strtotime( $headers['Age'] ) ) {
			$return['Age Headers'] = array(
				'icon'    => 'bad',
				'message' => __( 'The "Age" header is set to an invalid time, which will result in incorrect caching.', 'rmeote-cache-purge' ),
			);
		} else {
			$return['Age Headers'] = array(
				'icon'    => 'awesome',
				'message' => __( 'Your site is returning proper "Age" headers.', 'rmeote-cache-purge' ),
			);
		}
		...
		if ( isset( $headers['Pragma'] ) && strpos( $headers['Pragma'], 'no-cache' ) !== false ) {
			$return['Pragma Headers'] = array(
				'icon'    => 'bad',
				'message' => __( 'The header Pragma is set to to "no-cache" which means visitors will never get cached content.', 'rmeote-cache-purge' ),
			);
		}
		...
		if ( isset( $headers['X-Cache-Status'] ) && strpos( $headers['X-Cache-Status'], 'MISS' ) !== false ) {
			$return['X-Cache Satus'] = array(
				'icon'    => 'bad',
				'message' => __( 'X-Cache missed, which means your site was not able to serve this page as cached.', 'rmeote-cache-purge' ),
			);
		}
		...
		if ( isset( $headers['X-Mod-Pagespeed'] ) ) {
			if ( strpos( $headers['X-Cacheable'], 'YES:Forced' ) !== false ) {
				$return['Mod Pagespeed'] = array(
					'icon'    => 'good',
					'message' => __( 'Mod Pagespeed is active and configured to work properly with caching services.', 'rmeote-cache-purge' ),
				);
			} else {
				$return['Mod Pagespeed'] = array(
					'icon'    => 'bad',
					'message' => __( 'Mod Pagespeed is active but your caching headers may not be right. This may be a false negative if other parts of your site are overwriting headers. Fix all other errors listed, then come back to this. If you are still having errors, you will need to look into using .htaccess or Nginx to override the Pagespeed headers.', 'rmeote-cache-purge' ),
				);
			}
		}
	...
	public static function bad_themes_results() {
		...
		$return  = array();
		$request = wp_remote_get( 'https://rmeote-cache-purge.objects-us-east-1.dream.io/themes.json' );

		if ( is_wp_error( $request ) ) {
			if ( WP_DEBUG ) {
				$return['Theme Check'] = array(
					'icon'    => 'warning',
					'message' => __( 'Error: Theme data cannot be loaded.', 'rmeote-cache-purge' ),
				);
			}
		...
		if ( empty( $themes ) ) {
			if ( WP_DEBUG ) {
				$return['Theme Check'] = array(
					'icon'    => 'warning',
					'message' => __( 'Error: Theme data was empty.', 'rmeote-cache-purge' ),
				);
			}
			return $return; // Bail early.
		}
		...
		foreach ( $themes as $theme => $info ) {
			...
			if ( $my_theme->exists() ) {
				...
				if ( $active ) {
					$message .= __( 'Active', 'rmeote-cache-purge' );
				} else {
					$message .= __( 'Inactive', 'rmeote-cache-purge' );
					$warning  = 'notice';
				}
				$message .= ')';
		...
		if ( empty( $return ) ) {
			$return['Theme Check'] = array(
				'icon'    => 'good',
				'message' => __( 'No installed themes were found on the known conflicts list.', 'rmeote-cache-purge' ),
			);
		}
	...
	public static function bad_plugins_results() {
		...
		$return   = array();
		$messages = array(
			'incompatible' => __( 'This plugin has unexpected results with caching, making not function properly.', 'rmeote-cache-purge' ),
			'translation'  => __( 'Translation plugins that use cookies and/or sessions prevent most server side caching from running properly.', 'rmeote-cache-purge' ),
			'sessions'     => __( 'This plugin uses sessions, which conflicts with server side caching.', 'rmeote-cache-purge' ),
			'cookies'      => __( 'This plugin uses cookies, which may prevent server side caching.', 'rmeote-cache-purge' ),
			'cache'        => __( 'This type of caching plugin does not work well with server side caching.', 'rmeote-cache-purge' ),
			'ancient'      => __( 'This plugin is not up to date with WordPress best practices and breaks caching.', 'rmeote-cache-purge' ),
			'removed'      => __( 'This plugin was removed from WordPress.org and we do not recommend it\'s use.', 'rmeote-cache-purge' ),
			'maybe'        => __( 'This plugin is usually fine, but may be configured in a way that breaks caching. Please resolve all other errors. If this is the only one left, and caching is running, you may safely ignore this message.', 'rmeote-cache-purge' ),
		);

		$request = wp_remote_get( 'https://rmeote-cache-purge.objects-us-east-1.dream.io/plugins.json' );
		if ( is_wp_error( $request ) ) {
			if ( WP_DEBUG ) {
				$return['Plugin Check'] = array(
					'icon'    => 'warning',
					'message' => __( 'Error: Plugin data cannot be loaded.', 'rmeote-cache-purge' ),
				);
			}
			return $return; // Bail early.
		}
		...
		if ( empty( $plugins ) ) {
			if ( WP_DEBUG ) {
				$return['Plugin Check'] = array(
					'icon'    => 'warning',
					'message' => __( 'Error: Plugin data was empty.', 'rmeote-cache-purge' ),
				);
			}
			return $return; // Bail early.
		}
		...
		if ( empty( $return ) ) {
			$return['Plugin Check'] = array(
				'icon'    => 'good',
				'message' => __( 'No installed plugins were found on the known conflicts list.', 'rmeote-cache-purge' ),
			);
		}
	...
	public static function get_all_the_results( $headers, $remote_ip, $varniship ) {
		...
		if ( self::devmode_check() ) {
			$output['Development Mode'] = array(
				'icon'    => 'notice',
				'message' => __( 'NOTICE: Caching is disabled while Development Mode is active.', 'rmeote-cache-purge' ),
			);
		}
